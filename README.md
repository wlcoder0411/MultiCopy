...
[![Version](https://img.shields.io/badge/version-6.0-brightgreen.svg)](../../releases)

![MultiCopy 主界面（暗色主题）](docs/screenshots/origin.png)

MultiCopy 解决一个常见痛点：整理资料、填写表单、收集素材时，要在多个窗口间反复切换复制粘贴。它把你连续复制的每一段文本自动收集进队列，然后你可以按 `Ctrl+V` 一条条顺序粘贴，或点击任意条目选择性粘贴。支持文本与图片、置顶常驻、分组管理、粘贴前后缀与序号自增、明暗主题、托盘常驻与全局热键。

## 功能特性

- **多段连续采集**：后台监听剪贴板，每次复制自动入队，无需手动操作（队列上限 30 条，满后托盘气泡提示清理）
- **监控开关**：一键开启/关闭剪贴板监控。最小化到托盘时自动关闭监控（避免日常复制粘贴被拦截），恢复窗口显示时自动开启
- **队列粘贴**：开启「队列粘贴」模式后，`Ctrl+V` 逐条按顺序吐出队首内容
- **选择性粘贴**：单击列表项即可粘贴该条并出队
- **粘贴前后缀**：在设置中配置前缀/后缀内容（多行编辑，内置中文标点、空格、回车等预设），每次粘贴自动追加，前缀/后缀可独立开关
- **序号自增**：前缀支持自动递增序号，5 种格式（`1` / `01` / `(1)` / `1.` / `1、`）× 2 种位置（前缀前 / 前缀后），顶部栏一键重置序号
- **置顶常驻**：双击置顶项粘贴但不出队，置顶内容持久化保存（重启不丢）
- **分组管理**：QQ 风格分组，选中分组时复制入组、粘贴只消耗该组内容；右键条目支持「移动到分组」
- **关键字搜索**：在搜索框输入关键字快速过滤队列，文本按内容匹配、图片按来源应用和预览信息匹配（不区分大小写），`Esc` 一键清空
- **图片支持**：采集剪贴板图片（截图、复制的图片），列表显示 96×96 缩略图、尺寸与来源应用
- **明暗主题**：一键切换暗色 / 亮色主题
- **托盘常驻**：关闭窗口最小化到托盘，开机自启可选（自启时静默驻留托盘并关闭监控，不干扰日常复制粘贴）
...
| 序号自增与自定义前/后缀 | 前/后缀内容添加 |
| ![设置对话框](docs/screenshots/number.png) | ![设置对话框](docs/screenshots/context.png) |

## 下载安装

### 方式一：安装包（推荐普通用户）

前往 [Releases](../../releases) 下载 `MultiCopySetup-6.0.exe`，双击安装即可。安装包支持中文界面、桌面快捷方式、开机自启选项。

### 方式二：绿色免安装

前往 [Releases](../../releases) 下载绿色版可执行.exe文件，下载后双击 `MultiCopy.exe` 直接运行（无需安装 .NET 运行时，单文件自包含）。

> 单文件 exe （已内嵌 .NET 8 WPF 运行时），首次启动会解压到临时目录，慢 1–3 秒属正常。
...
- 设置：`%APPDATA%\MultiCopy\settings.json`（含全局热键、粘贴前缀/后缀、序号自增配置）
