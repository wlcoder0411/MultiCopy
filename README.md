...
[![Version](https://img.shields.io/badge/version-3.6-brightgreen.svg)](../../releases)
...
- **按序粘贴**：开启「按序粘贴」模式后，`Ctrl+V` 逐条吐出队首内容
...
前往 [Releases](../../releases) 下载 `MultiCopySetup-3.6.exe`，双击安装即可。安装包支持中文界面、桌面快捷方式、开机自启选项。

### 方式二：绿色免安装

前往 [Releases](../../releases) 下载 zip 压缩包，解压后双击 `MultiCopy.exe` 直接运行（无需安装 .NET 运行时，单文件自包含）。
...
```powershell
git clone https://github.com/WX041119/MultiCopy.git
cd MultiCopy
...
```powershell
.\publish.ps1               # 生成绿色版 zip + 安装包（需 Inno Setup 6）
.\publish.ps1 -SkipInstaller  # 仅生成绿色版 zip
```
